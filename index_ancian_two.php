        ...
        <div class="modal fade" id="modaltest" aria-hidden="true" aria-labelledby="exampleModalToggleLabel" tabindex="-1">
          <div class="modal-dialog modal-lg modal-dialog-centered">
            <div class="modal-content container">
              ...
              <script>
              /* ---------- AJAX: إرسال الفورم العام إلى test/submit_test.php ---------- */
              document.getElementById('testform').addEventListener('submit', function(e){
                e.preventDefault();
                const formData = new FormData(this);
                const resultDiv = document.getElementById('test-result');

                fetch('test/submit_test.php', { method: 'POST', body: formData })
                  .then(r => r.text())
                  .then(html => {
                    // عرض comment_msg + الأزرار (main result)
                    resultDiv.innerHTML = html;
                    // نحتفظ بالـ main HTML كي نعود إليه عند الحاجة
                    window.__mainResultHTML = resultDiv.innerHTML;
                    // scroll داخل modal للنتيجة
                    resultDiv.scrollIntoView({behavior:'smooth'});
                  })
                  .catch(err => {
                    resultDiv.innerHTML = '<div class="alert alert-danger">خطأ في الإرسال.</div>';
                    console.error(err);
                  });
              });


              /* ---------- تحميل forms الفرعية وارسالها ديناميكياً ---------- */
              document.addEventListener('click', function(e){
                // زر فتح اختبار فرعي (من الأزرار التي يولّدها submit_test.php)
                if(e.target.classList.contains('load-subtest-btn')){
                  e.preventDefault();
                  const file = e.target.getAttribute('data-file');   // مثال: 'phq.php'
                  const base = e.target.getAttribute('data-base');   // مثال: 'data_phq'
                  const baseId = e.target.getAttribute('data-id');   // id في test_base
                  const container = document.getElementById('test-result');

                  // تحميل الفورم الفرعي عبر GET مع id
                  fetch('test/' + file + '?id=' + encodeURIComponent(baseId))
                    .then(r => r.text())
                    .then(html => {
                      // حقن الفورم داخل نفس الـ container، وضع div للنتيجة الخاصة بالاختبار
                      container.innerHTML = html + '<div id="test-result_' + base + '" class="mt-3"></div>' + '<div class="mt-3">' + window.__mainResultHTML.replace(/\n/g,'') + '</div>';
                      // ملاحظة: الجزء الأخير يعيد عرض بقية الأزرار (mainResultHTML يحتوي الأزرار الأصلية)؛ يمكنك تحسين العرض حسب التنسيق الذي تريده.
                    })
                    .catch(err => {
                      container.innerHTML = '<div class="alert alert-danger">فشل تحميل الاختبار.</div>';
                      console.error(err);
                    });
                }

                // زر العودة (سيظهر داخل كل نتيجة اختبار) لإعادة الشاشة الرئيسية للنتائج
                if(e.target && e.target.id === 'backToMainResult'){
                  e.preventDefault();
                  const container = document.getElementById('test-result');
                  container.innerHTML = window.__mainResultHTML || '';
                }
              });
              </script>
            </div>
